...
//启动聊天窗口并连接聊天服务器
 function creatw() {
			...
			//初始话WebSocket
			function initWebSocket() {debugger
				if (window.WebSocket) {
					websocket = new WebSocket(encodeURI('ws://'+wimadress));
					
					websocket.onopen = function() {debugger
						//连接成功
						win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
						websocket.send('Helloadminqq651521191'+user);
					}
					websocket.onerror = function() {debugger
						//连接失败
						win.setTitle(title + '&nbsp;&nbsp;(连接发生错误)');
					}
					websocket.onclose = function() {debugger
						//连接断开
						win.setTitle(title + '&nbsp;&nbsp;(已经断开连接)');
					}
					//消息接收
					websocket.onmessage = function(message) {debugger
			...
			win.on("close",function(){
				websocket.send('LeaveHelloadminqq651521191');
				isCreatw = false;
			 });
			...
			function send() {
				...
				if(toUser != ""){content = "helloadmin996"+toUser+"hellohelloadmin998" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
...
Ext.define('MessageContainer', {
	...
	tpl : [
			'<div class="l-im-message-warn">​欢迎使用Hello Admin 即时通讯系统。</div>',
			'<tpl for=".">',
