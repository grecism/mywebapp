...
if ( contentType.indexOf("multipart/form-data") >= 0 )
{
	...
	while( fileItems.hasNext() )
	{
		...
		{
			//realPath+"upload"+File.separator+"user"+File.separator+ ((Userbasicsinfo) request.getSession().
					//getAttribute(Constant.SESSION_USER)).getId() + File.separator;
			String virtualPath = "uploadFiles"+File.separator+"uploadUserPhoto"+File.separator+"jsp_avatar" + avatarNumber + "_" + fileName + ".jpg";
			...
			if( isSourcePic )
			{
				...
				result.sourceUrl = virtualPath = String.format("uploadFiles"+File.separator+"uploadUserPhoto"+File.separator+"jsp_source_%s.%s", fileName, sourceExtendName);
			}
			...
			inputStream = new BufferedInputStream(fileItem.openStream());
			System.out.println(">>>后台上传图片路径："+application.getRealPath("/") +File.separator+ virtualPath);
			outputStream = new BufferedOutputStream(new FileOutputStream(application.getRealPath("/") +File.separator+ virtualPath));
