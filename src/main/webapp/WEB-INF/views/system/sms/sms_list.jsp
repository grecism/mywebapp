...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="sms/list.do" method="post" name="Form" id="Form">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:20px;"><a href="sms/list.do?TYPE=1"><span class="label label-<c:if test="${pd.TYPE != '2' }">success</c:if> arrowed-right arrowed-in">收信箱</span></a></td>
								<td><a href="sms/list.do?TYPE=2"><span class="label label-<c:if test="${pd.TYPE == '2' }">info</c:if> arrowed-right arrowed-in">发信箱</span></a></td>
							</tr>
						</table>
						<!-- 检索  -->
	...
	<script type="text/javascript">
		...
		function sendFhsms(username){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '<%=basePath%>sms/goAdd.do?username='+username;
			 diag.Width = 660;
			 diag.Height = 444;
		...
		function del(ztid,STATUS,type,Id,SANME_ID){
			bootbox.confirm("确定要删除吗?", function(result) {
				...
				if(result) {
					top.jzts();
					var url = "<%=basePath%>sms/delete.do?SMS_ID="+Id+"&tm="+new Date().getTime();
		...
		function viewx(ztid,STATUS,type,Id,SANME_ID){
			if(type == "1" && STATUS == '2' && $("#"+ztid).html() == '<span class="label label-important arrowed-in">未读</span>'){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = '<%=basePath%>sms/goView.do?SMS_ID='+Id+'&TYPE='+type+'&SANME_ID='+SANME_ID+'&STATUS='+STATUS;
			 diag.Width = 600;
			 diag.Height = 460;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>sms/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
