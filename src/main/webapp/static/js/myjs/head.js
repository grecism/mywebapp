...
function getHeadMsg(){
	$.ajax({
		...
		success: function(data){
			 ...
			 fhsmsCount = Number(data.smsCount);
...
function getFhsmsCount(){
	$.ajax({
		type: "POST",
		url: locat+'/head/getSmsCount.do?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
		cache: false,
		success: function(data){
...
//下线
function goOut(msg){
	window.location.href=locat+"/adminLogout.do?msg="+msg;
}
...
function fhsmsmsg(USERNAME){
	...
	for(var i=0;i<arrUSERNAME.length;i++){
		websocket.send('[sms]'+arrUSERNAME[i]);//发送通知
	}
}
...
function fhsms(){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = locat+'/sms/list.do?STATUS=2';
	 diag.Width = 800;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
