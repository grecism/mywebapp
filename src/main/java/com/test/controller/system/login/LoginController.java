package com.test.controller.system.login;
...
import util.admin.DateUtil;
import util.admin.PageData;
import util.admin.RightsHelper;
...
public class LoginController extends BaseController {
	...
	public ModelAndView loginIndex(HttpServletRequest request, HttpServletResponse response,UserAdmin userAdmin) throws Exception{
	Properties p = new Properties();
	p.getProperty("1");
		return new ModelAndView("/login/login_index");
	}
	/**访问登录页
	 * @return
	 * @throws Exception
	...
	public Object login()throws Exception{
		...
		System.out.println("1111");
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("qq313596790hello", "").replaceAll("QQ978336446hello", "").split(",hello,");
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
						...
						pd.put("CURRENT_REVIEW_COUNT", Integer.valueOf(pd.get("CURRENT_REVIEW_COUNT").toString())+1);
						...
						try {
							//LOG.save(USERNAME, "登录系统密码或用户名错误");
						} catch (Throwable e) {
							logger.error(e.toString(), e);
						}
					}
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				...
				if(null == userr){
					user = (User) userService.getUserAndRoleById(user.getUSER_ID());				//通过用户ID读取用户信息和角色信息
				...
				if(null == session.getAttribute(USERNAME + Const.SESSION_QX)){
					logger.info(this.getUQX(USERNAME));
				...
				logger.info(">>>登录角色对应的菜单列表"+JSONObject.toJSONString(menuList));
			}else {
	...
	public PageData setLoginPd(PageData pd){
		...
		try {
			...
			pd.put("listImg", Collections.emptyList());
		} catch (Exception e) {
			e.printStackTrace();
		}catch (Throwable e) {
			logger.error(e.toString(), e);
