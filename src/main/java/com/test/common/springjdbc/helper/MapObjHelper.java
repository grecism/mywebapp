...
public class MapObjHelper {
	...
	public static <K, V>List<Map<K, V>> transAndFlushListMap(List<Map<K,V>>K_VList,List<V[]>VS){
		if(CollectionUtils.isEmpty(K_VList)){
			return K_VList;
		}
	...
	public static  <K, V> Map<K, V> transAndFlushMap(Map<K, V> K_VMap,List<V[]>VS) {
		if(CollectionUtils.isEmpty(K_VMap)){
			return K_VMap;
		}
				...
				if(v == null){
					...
					if(!ObjectUtils.isEmpty(vs[3])){
						try {
							...
							v = (V) InvokeMethod.invoke(invokeArgs[0]+"#"+invokeArgs[1], new Class[]{Object.class,Object.class},invokeArgs[2], v);
						} catch (Exception e) {
							v = vs[2];
						}
					}
